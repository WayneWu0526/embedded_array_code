...
Personal research and implementation notes migrated from the root `TODO.md`.

## 2026-05-24 Notes

要将R_CORR改到直接读取后就可以计算了，也就是在读取侧就直接获取旋转之后的结果，raw数据就应该默认进行一次旋转（这个很重要）
...
4. 将是否开启某个chanel作为launch参数，例如chanel1值为true表示启动chanel1，chanel2,3,false表示他们不启动。将每个chanel的外部施加的理论磁场模值也作为launch参数传入。
5. 开始之前需要终端汇报具体的参数设置，例如chanel1,2,3是否开启，施加的理论磁场模值分别是多少。
