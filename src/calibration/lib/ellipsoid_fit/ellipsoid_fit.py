...
def ellipsoid_fit(b_raw: np.ndarray, sensor_id: int,
                  ...
                  o_i: np.ndarray = None,
                  C_i: np.ndarray = None) -> CalibrationResult:
    """
    单颗传感器椭球拟合或评估

    两种模式:
    1. 拟合模式（默认）: 输入原始数据，拟合得到 o_i, C_i，计算评价指标
    2. 评估模式: 输入原始数据 + 预计算 o_i, C_i，直接计算评价指标

    Args:
        b_raw: N×3 原始磁场采样数据
        sensor_id: 传感器编号 (1-12)
        csv_file: 原始数据文件名
        use_iterative: 是否使用迭代方法（仅拟合模式有效）
        o_i: (3,) 预计算的 offset 向量（可选，提供则进入评估模式）
        C_i: (3×3) 预计算的校正矩阵（可选，提供则进入评估模式）

    Returns:
        CalibrationResult: 校准/评估结果
    """
    # 判断模式
    evaluate_only = o_i is not None and C_i is not None

    if evaluate_only:
        # 评估模式：使用预计算参数，跳过拟合
        o_i = np.asarray(o_i)
        C_i = np.asarray(C_i)
        fit_info = {"method": "precomputed", "mode": "evaluation"}

        # 计算 eigenvalue ratio（仅用于参考，不做主要评价指标）
        cov = np.cov(b_raw.T)
...
def batch_ellipsoid_fit(csv_path: Path, output_dir: Path = None,
                        ...
                        evaluate_only: bool = False) -> List[CalibrationResult]:
    """
    批量处理单个 CSV 文件中所有传感器的校准或评估

    两种模式:
    1. 校准模式（默认）: 输入原始数据，拟合得到 o_i, C_i，计算评价指标
    2. 评估模式: 输入原始数据 + 预计算参数，直接计算评价指标
    ...
    Args:
        ...
        sensor_type: 传感器型号（可选，如 "QMC6309"），指定后自动从配置加载参数
        evaluate_only: 是否仅评估模式（使用预计算参数，不拟合）
    ...
    mode_str = "evaluation" if evaluate_only else "calibration"
    print(f"[INFO] Running Phase 1 (ellipsoid) {mode_str}...")

    # 如果是评估模式，预加载参数
    intrinsic_params = {}
    if evaluate_only:
        intrinsic_params = {
            sid: (np.array(p.o_i), np.array(p.C_i))
            for sid, p in sensor_config.intrinsic.params.items()
        }
    ...
    for sensor_id in sensor_config.get_sensor_ids():
        ...
        # 执行校准或评估
        if evaluate_only:
            if sensor_id not in intrinsic_params:
                print(f"[WARN] Sensor {sensor_id}: no intrinsic params found, skipping")
                continue
            o_i, C_i = intrinsic_params[sensor_id]
